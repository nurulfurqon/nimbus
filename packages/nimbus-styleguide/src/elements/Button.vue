<template>
  <component :is="type" :href="href" :type="submit" :class="['button', size, state, variation]">
    <slot/>
    <div style="display:block; width:10px; height:10px; background:#ddd;" :style="{ backgroundColor:dataTokens.colorUraniumDark }">
    </div>
  </component>
</template>

<script>
import dataTokens from '@nimbus/nimbus-tokens';
/**
...
@import "@nimbus/nimbus-tokens/dist/color/scss/color.scss";
.button {
  @include reset;
